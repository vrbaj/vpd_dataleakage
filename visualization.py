...
with open("results_scaling_leakage_0_200_dt.json", "r") as f:
    data_l = json.load(f)
mcc_differences = []

for item in data_l.values():
    if item["leakage"] != 0 and item["correct"] != 0:
        mcc_differences.append(item["leakage"] - item["correct"])

check_normality(np.array(mcc_differences))
print(f"average difference is: {np.average(np.array(mcc_differences))}" )
...
plt.ylabel("Count")
plt.title(f"Decision tree + MinMax")
